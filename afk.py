...
from strings import cmd
from utils import get_readable_time
from utils.database import cleanmode, cleanmode_off, cleanmode_on, dB
    ...
    if reply_markup is None:
        if afktype == "animation":
            ...
            return await message.reply_photo(
                photo=get_media_path(user_id, afktype), caption=caption
            )
        elif afktype == "video":
            return await message.reply_video(
                video=get_media_path(user_id, afktype), caption=caption
            )
        else:
        ...
        if afktype == "animation":
            return await message.reply_animation(
                data, caption=caption, reply_markup=reply_markup
            )
        elif afktype == "photo":
            return await message.reply_photo(
                photo=get_media_path(user_id, afktype),
                caption=caption,
                reply_markup=reply_markup,
            )
        elif afktype == "video":
            return await message.reply_video(
                video=get_media_path(user_id, afktype),
                caption=caption,
                reply_markup=reply_markup,
            )
        else:
            return await message.reply_text(
                caption, disable_web_page_preview=True, reply_markup=reply_markup
            )


async def handle_afk_reply(
    message,
    afktype,
    user_id,
    user_mention,
    ...
    return await reply_afk_message(
        message, afktype, data, caption, user_id, reply_markup
    )


@cmd(["afk", "brb"])
async def active_afk(_, message, langs):
...
@cmd("afkdel", only_group=True, admins=True)
async def afkdel_state(_, message, langs):
...
async def afk_watcher_func(client, message):
    ...
    full_text = f"{message_text} {message_caption}".lower()
