...
@cmd("BINGAI_COMMAND", checklimit="bingquery")
async def bingai_cmd(client, message, _):
    user_id = message.from_user.id if message.from_user else None
    if not user_id or message.sender_chat:
        return await message.reply_text(_["general_4"])
    prompt = client.get_text(message)
    if not prompt:
        return await message.reply(_["bingai_1"].format(message.text.split()[0]))
    pros = await message.reply(_["bingai_2"].format(prompt))
        ...
        if imgs:
            ...
            for img in imgs:
                if os.path.exists(img):
                    ...
                    media_group.append(
                        types.InputMediaPhoto(media=img, caption=caption)
                    )
            ...
            return await pros.edit(_["bingai_4"])
    except Exception as e:
        error_message = str(e)
        LOGGER.error(f"Bing error: {traceback.format_exc()}")
        if "Failed to decode" in error_message:
            return await pros.edit(_["bingai_5"])
        else:
            return await pros.edit(_["general_7"].format(error_message))
    return
